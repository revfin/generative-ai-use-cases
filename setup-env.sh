...
export VITE_APP_AGENT_RUNTIME_FUNCTION_ARN=$(extract_value "$stack_output" MimirAgentFunctionArn)
export VITE_APP_AGENT_RUNTIME_ENABLED=$([ -n "$VITE_APP_AGENT_RUNTIME_FUNCTION_ARN" ] && echo true || echo false)
export VITE_APP_MIMIR_MEMORY_ENABLED=$([ -n "$(extract_value "$stack_output" MimirMemoryFunctionArn)" ] && echo true || echo false)
